...
LIBRARY_TESTS="error notify sector_range support track_value";
...
run_test()
{
	...
	local TEST_EXECUTABLE="./smdev_test_${TEST_NAME}";
...
run_test_with_input()
{
	...
	run_test_on_input_directory "libsmdev" "${TEST_DESCRIPTION}" "default" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "input" "${INPUT_GLOB}";
	local RESULT=$?;

       ...
       size_t size )
{
	...
	if( smdev_test_real_malloc == NULL )
	{
		smdev_test_real_malloc = dlsym(
		                          RTLD_NEXT,
		                          "malloc" );
	}
	if( smdev_test_malloc_attempts_before_fail == 0 )
	{
		smdev_test_malloc_attempts_before_fail = -1;

		return( NULL );
	}
	else if( smdev_test_malloc_attempts_before_fail > 0 )
	{
		smdev_test_malloc_attempts_before_fail--;
	}
	ptr = smdev_test_real_malloc(
	       size );
       ...
       size_t size )
{
	...
	{
		smdev_test_real_memcpy = dlsym(
		                          RTLD_NEXT,
		                          "memcpy" );
	}
	if( smdev_test_memcpy_attempts_before_fail == 0 )
	{
		smdev_test_memcpy_attempts_before_fail = -1;

		return( NULL );
	}
	else if( smdev_test_memcpy_attempts_before_fail > 0 )
	{
		smdev_test_memcpy_attempts_before_fail--;
	}
	destination = smdev_test_real_memcpy(
	               destination,
	               source,
	               size );
...
void *memset(
       ...
       size_t size )
{
	if( smdev_test_real_memset == NULL )
	{
		smdev_test_real_memset = dlsym(
		                          RTLD_NEXT,
		                          "memset" );
	}
	if( smdev_test_memset_attempts_before_fail == 0 )
	{
		smdev_test_memset_attempts_before_fail = -1;

		return( NULL );
	}
	else if( smdev_test_memset_attempts_before_fail > 0 )
	{
		smdev_test_memset_attempts_before_fail--;
	}
	ptr = smdev_test_real_memset(
	       ptr,
	       constant,
	       size );
...
void *realloc(
       ...
       size_t size )
{
	if( smdev_test_real_realloc == NULL )
	{
		smdev_test_real_realloc = dlsym(
		                           RTLD_NEXT,
		                           "realloc" );
	}
	if( smdev_test_realloc_attempts_before_fail == 0 )
	{
		smdev_test_realloc_attempts_before_fail = -1;

		return( NULL );
	}
	else if( smdev_test_realloc_attempts_before_fail > 0 )
	{
		smdev_test_realloc_attempts_before_fail--;
	}
	ptr = smdev_test_real_realloc(
	       ptr,
	       size );
